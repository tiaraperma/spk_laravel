...
class AdminAlternativeController extends Controller
{
  public function index()
  {
    ...
    $usedIds    = Alternative::select('criteria_id')->distinct()->get();
    $usedIdsFix = [];

    foreach ($usedIds as $usedId) {
      array_push($usedIdsFix, $usedId->criteria_id);
    }

    $alternatives = TourismObject::with('alternatives')
      ->get();
